...
> ## Статус проекта
>
> Версия **2.7.1**. Дефекты аудита 2.5.7 закрыты в 2.6.0. Версия 2.7.0 — про интерфейс и готовность к постоянной работе. Версия 2.7.1 починила тесты интерфейса на реальном aiogram.
> Код проходит 78 тестов: 73 без принтера и 5 для конвертера. Установщик прогнан на хосте принтера (Armbian): выпуск 2.7.0 остановился на тестах и откатился сам, 2.7.1 эту причину устранил. Первый запуск делайте с `NO_SERVICE=1`.
>
...
Сейчас в наборе 78 тестов: 11 в `tests/test_core.py`, 20 в `tests/test_fixes.py`, 4 в `tests/test_safety_async.py`, 38 в `tests/test_ux.py`, 5 в `tests/test_converter.py`.
Тесты интерфейса строят события на настоящих моделях aiogram, поэтому расхождение с установленной библиотекой видно сразу.
